...
- ⭐ **Избранные маршруты** — сохранение частых маршрутов для быстрого доступа (1 клик)
- 📈 **Личная статистика** — отслеживание поисков, бронирований и сэкономленных средств
- 🧪 **A/B тестирование** — feature flags для тестирования новых функций
- 🔗 **Webhook интеграция** — логирование событий для внешних систем
- 🎯 **Контекстное меню** — быстрые действия под результатами поиска
- 🎫 **Быстрая покупка** — кнопка перехода на сайт RW.BY с предзаполненными параметрами
...
| `/mytracks` | Показать все активные трекинги пользователя с кнопками удаления |
| `/history` | Показать историю последних поисков |
| `/favorites` | ⭐ Избранные маршруты для быстрого доступа |
| `/stats` | 📈 Персональная статистика пользователя |
| `/admin` | 🔐 Панель администратора (только для пользователей с ролью admin/moderator) |
| `/help` | Подробная справка по использованию бота |

### Админ-функции в боте

Бот поддерживает ролевую модель с тремя уровнями доступа:
...
После ввода команды `/admin` пользователи с ролью `admin` или `moderator` получают доступ к меню:

- **📊 Админ-статистика** — общая статистика по пользователям, трекингам и поискам
- **📜 Просмотр логов** — последние действия всех пользователей с деталями
- **👥 Пользователи** — список последних пользователей с их ролями и активностью

Для назначения роли администратора используйте SQL-запрос напрямую в базе данных:

```sql
UPDATE users SET role = 'admin' WHERE chat_id = YOUR_CHAT_ID;
```

Узнать свой chat_id можно через бота @userinfobot или посмотрев логи бота.
...
├── admin_panel.py      # Веб-админка для управления трекингами
...
├── README.md          # Этот файл
├── ADMIN_PANEL_README.md  # Документация по админ-панели
└── SYNC_INTEGRATION.md    # Документация по синхронизации
```
...
- `active_trackings` — активные трекинги билетов (с уникальным ID для каждого трекинга)
- `search_history` — история поисков пользователей
- `popular_stations` — популярные станции для быстрого выбора
- `sync_flags` — флаги синхронизации между ботом и админ-панелью
- `request_counter_history` — история завершенных трекингов с количеством запросов
- `favorite_routes` — ⭐ избранные маршруты пользователей для быстрого доступа
- `user_stats` — 📈 статистика пользователей (поиски, бронирования, сэкономленные средства)
- `feature_flags` — 🧪 флаги функций для A/B тестирования
- `webhook_events` — 🔗 события для webhook интеграции с внешними системами
...
- ✅ Уникальная идентификация каждого трекинга (удаление только конкретного трека)
- ✅ Синхронизация удаления между админ-панелью и ботом
- ✅ Сохранение счетчика запросов (`requests_count`) при перезапусках
- ✅ ⭐ Быстрый доступ к избранным маршрутам
- ✅ 📈 Персональная статистика для каждого пользователя
- ✅ 🧪 Гибкое управление функциями через A/B тесты
